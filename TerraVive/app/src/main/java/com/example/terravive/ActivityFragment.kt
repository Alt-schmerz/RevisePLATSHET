...
class ActivityFragment : Fragment() {
        ...
        val activityAdapter = object : RecyclerView.Adapter<ActivityViewHolder>() {
            ...
            override fun onBindViewHolder(holder: ActivityViewHolder, position: Int) {
                ...
                val userDocRef = db.collection("activities")
                    .document(item.id)  // Using id here since it's the activities collection
                    .collection("users")
                    .document(userEmail)
    ...
    private fun reserveUserUnderActivity(item: ActivityEvent, userEmail: String, holder: ActivityViewHolder) {
        ...
        // Store the user reservation under the 'users' subcollection of each activity
        val activityRef = db.collection("activities")
            .document(item.id)  // Using 'activities' collection here to store details

        // First, ensure that the activity info (title, time, location, description) is added to Firestore
        val activityDetails = hashMapOf(
            "title" to item.name,
            "time" to item.time,
        ...
        // Add or update the activity details if not already stored
        activityRef.set(activityDetails, SetOptions.merge())  // Merge ensures we don't overwrite data

        // Now, add the user's reservation under the 'users' subcollection
        activityRef.collection("users")
            .document(userEmail)
            .set(userData)
            .addOnSuccessListener {
                ...
                holder.btnReserve.setOnClickListener {
                    // Undo reservation by deleting the user entry
                    activityRef.collection("users")
                        .document(userEmail)
                        .delete()
                        .addOnSuccessListener {
